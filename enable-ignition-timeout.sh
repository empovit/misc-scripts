...
HOSTS=`curl -sSf -X GET -H "Content-Type: application/json" -H "authorization: ${BEARER}" "${CLUSTER_URL}" | jq "." | grep '"id"' | grep -v "$CLUSTER" | awk -F'"' '{print $4}'`
...
do
    ...
    curl -X PATCH \
        ...
        --data-binary '{"config": "{\"ignition\":{\"timeouts\":{\"httpTotal\":120},\"version\":\"3.2.0\"}}"}' \
    ...
    curl -X PATCH \
        -H "Content-Type: application/json" \
        --data-binary '{"args": ["--append-karg", "rd.shell", "--append-karg", "rd.debug"]}' \
        -H "authorization: Bearer ${BEARER}" \
